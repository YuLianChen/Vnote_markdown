...
- **自动 Group 管理（物理键适配）：**
    ...
    - 当 `onEnter` 触发时，Navigator 自动清除旧 Group 的对象，并对象数组将当前页面的需要添加的交互组件（按钮、滑块）加入 Group，自动聚焦第一个组件，无需开发者手动调用 `lv_group_add_obj`。
...
1. **线程安全的双向数据总线**
	...
	- **命令结果回传（Backend → UI）：** 将命令的执行结果回传到UI
        
2. **状态快照与参数恢复**
	1. **状态中心 StateStore：** 储存全局的系统内状态信息，同时存在读写两个内存缓冲区，获取状态信息使用内存拷贝的方式，保证获取到的状态信息是完整的的
